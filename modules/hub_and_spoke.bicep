/**
  Deploy's hub for a hub-spoke network.

  This is copied from:
  https://github.com/mspnp/samples/tree/main/solutions/azure-hub-spoke

  Modifications:
  1. fix upstreamed as PR #202
  2. Use existing log analytics workspace and make it optional
  3. Change spoke 1 & 2 addressPrefix to '.../16' instead of '../22'
  4. Expand hub prefix and add snet-resources subnet for jump-boxes etc.
  5. add pool subnet (TODO: avoid this)
  6. update resource-nsg
  7. update firewall rules to allow internet access
*/
...
var enableLogAnalytics = logAnalyticsWorkspaceId != '' ? true : false
...
resource nsgBastionSubnet_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
  ...
  properties: {
    ...
    subnets: [
      ...
      }
      {
        name: 'GatewaySubnet'
        properties: {
          addressPrefix: '10.201.2.0/27'
        }
      }
      {
        name: 'AzureFirewallSubnet'
        properties: {
          addressPrefix: '10.201.3.0/26'
        }
      }
      {
        name: 'snet-resources'
        properties: {
          addressPrefix: '10.201.4.0/24'
        }
      }
    ]
  }
...
resource vnetHub_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
  name: 'to-hub-la'
  scope: vnetHub
  properties: {
    workspaceId: logAnalyticsWorkspaceId
...
resource pipsAzureFirewall_diagnosticSetting 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = [for i in range(0, numFirewallIpAddressesToAssign): if (enableLogAnalytics) {
  name: 'to-hub-la'
...
resource fwHub_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
  name: 'to-hub-la'
  scope: fwHub
  properties: {
    workspaceId: logAnalyticsWorkspaceId
...
resource pipAzureBastion_diagnosticSetting 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
  name: 'to-hub-la'
...
resource azureBastion 'Microsoft.Network/bastionHosts@2022-01-01' = {
  ...
  location: location
  sku: {
    name: 'Standard' // 'Basic' cannot be used since we needed linux-rdp
  }
  properties: {
    ipConfigurations: [
      {
...
resource azureBastion_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
  name: 'to-hub-la'
  scope: azureBastion
  properties: {
    workspaceId: logAnalyticsWorkspaceId
...
resource pipVpnGateway_diagnosticSetting 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (deployVpnGateway && enableLogAnalytics) {
  name: 'to-hub-la'
  scope: pipVpnGateway
  properties: {
    workspaceId: logAnalyticsWorkspaceId
...
resource vgwHub_diagnosticSetting 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (deployVpnGateway && enableLogAnalytics) {
  name: 'to-hub-la'
  scope: vgwHub
  properties: {
    workspaceId: logAnalyticsWorkspaceId
...
resource nsgResourcesSubnet_diagnosticsSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
...
resource nsgPrivateLinkEndpointsSubnet_diagnosticsSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
...
resource vnetSpokeOne_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
  scope: vnetSpokeOne
  name: 'to-hub-la'
  properties: {
    workspaceId: logAnalyticsWorkspaceId
...
resource nicVmSpokeOneLinux_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (deployVirtualMachines && enableLogAnalytics) {
...
resource vnetSpokeTwo_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (enableLogAnalytics) {
  scope: vnetSpokeTwo
  name: 'to-hub-la'
  properties: {
    workspaceId: logAnalyticsWorkspaceId
...
resource nicVmSpokeTwoLinux_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = if (deployVirtualMachines && enableLogAnalytics) {
...
output vnetHub object = {
  name: vnetHub.name
  id: vnetHub.id
  snetResources: {
    name: vnetHub::snetResources.name
    id: vnetHub::snetResources.id
  }
}

output vnetSpokeOne object = {
  name: vnetSpokeOne.name
  id: vnetSpokeOne.id
  snetResources: {
    name: vnetSpokeOne::snetResources.name
    id: vnetSpokeOne::snetResources.id
  }
  snetPool: {
...
output vnetSpokeTwo object = {
  name: vnetSpokeTwo.name
  id: vnetSpokeTwo.id
  snetResources: {
    name: vnetSpokeTwo::snetResources.name
    id: vnetSpokeTwo::snetResources.id
  }
}
